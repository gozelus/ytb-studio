/**
 * [WHAT] End-to-end prompt quality eval: feeds a real YouTube transcript through
 *        buildPrompt + real Gemini, then asserts the ndjson output has the same
 *        structural shape as the笔试题 sample (h2 chapters, speakers Jen/Mark/John,
 *        rewrite style章节划分).
 *
 * [WHY]  buildPrompt is a string — unit tests only prove the text assembles.
 *        Only a real LLM call proves the prompt actually steers Gemini toward the
 *        target output. Gating on GEMINI_API_KEY keeps CI green without secrets.
 *
 * [INVARIANT] Opt-in via env: the suite is skipped whenever GEMINI_API_KEY is missing.
...
const apiKey = (cfEnv as { GEMINI_API_KEY?: string }).GEMINI_API_KEY
...
async function runPromptOnce(mode: 'rewrite' | 'faithful') {
  ...
  const prompt = buildPrompt(mode, META, TRANSCRIPT)
